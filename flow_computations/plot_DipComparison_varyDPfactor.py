...
# (i.e. the model used to create the DP file, which is here used to compute dips)
iseg_min=0; shift_edges=0; epslrc = 100.e3; eps_fact = 0.01  
epsdp_fact = 2*eps_fact; alith = 80.0e3; ah1=660.*1.e3; rad_km = 6378.;
#################################################################################
...
if flux_slab == 1:
	flux_vel_const = 50.    # [mm/yr]
	plot_string = ''.join([plates,'.',str(amu),'ConstFlux_width',str(flux_width),'_v',str(flux_vel_const),'_alpha',str(flux_alpha),tail_flux_string]);
	...
	plot_string = ''.join([plates,'.',str(amu),'_VcSlabFlux_width',str(flux_width),'_alpha',str(flux_alpha),tail_flux_string]);
	text_string = ''.join(['text_files/',plates,'.',str(amu),'_VcSlabFlux_width',str(flux_width),'_alpha',str(flux_alpha),tail_flux_string]);
else:
	plot_string = ''.join([plates,'.',str(amu),'noslabflux',tail_flux_string]);
...
n_avg = 0; rms_sum_avg = 0; mean_sum_avg = 0
...
plot_name_png = ''.join(['plots/dip_comparisons/',plot_string,'.fact',str(best_factor_rounded),'.png'])
...
ax.text(0.4,0.13,rms_all_string,size=5.5, color="black",transform = ax.transAxes)
mean_all_string = ''.join(['mean = ',str(best_mean_all),'$^\circ$, $\mathregular{mean_{avg}}$ = ',str(mean_avg),'$^\circ$'])
ax.text(0.4,0.06,mean_all_string,size=5.5, color="black",transform = ax.transAxes)
