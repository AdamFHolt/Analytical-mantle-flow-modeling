...
def outputgrids(spacing,lona,lata,lonb,latb,lono,lato,iwall,gam,alpha,amu,a,n_segs,num_segs,pcoeff,\
	...
	return (P_grd*P_zfactor, Pwall_grd*P_zfactor, Pedge_grd*P_zfactor, dPdlon_grd*P_zfactor, dPdlat_grd*P_zfactor, polygon_points, plate_vel_ew, plate_vel_ns, trench_vels, avgvel_asthen_ew, avgvel_asthen_ns, lon_grd, lat_grd, polygons)



def outputgrids_more(spacing,lona,lata,lonb,latb,lono,lato,iwall,gam,alpha,amu,a,n_segs,num_segs,pcoeff,\
		rad_km,domain_bounds,bound_ind,pole_top_lon,pole_top_lat,pole_top_rate,vt_ew,vt_ns,alith,press_depth,\
		coefftr1,coefftr2,pole_bott_lon,pole_bott_lat,pole_bott_rate,rigid_vew,rigid_vns,ah1,ndomain):
...
def output_trenchvels(lono,lato,lona,lata,lonb,latb,iwall,gam,alpha,ah1,amu,n_segs,num_segs,pcoeff,thresh_dist_wall,rad_km,polarity,vtopl,vtopr,vbotl,vbotr,
	vt,rigid_vew,rigid_vns,taper_length,alith,coefftr1,coefftr2,ndomain,idl,idr,epssub_fact,large_wall_inds):

	# Check that we can recover the initially imposed trench velocities
	# (A good check for the simpler models)

	vel_convert = 100.*60.*60.*24.*365.
	predicted_vts = np.zeros((n_segs,6)) 

	for jobs in range(n_segs):  # segments where velocity/pressure is calculated

		gamma = gam[jobs];
		elit = epssub_fact * gamma
		ebig = elit + 1.e3
		len_ratio_big = ebig/(rad_km*1e3)
		len_ratio_lit = elit/(rad_km*1e3)

		if iwall[jobs] == 1  or jobs >= num_segs:

			deriv_left_tot = 0;  deriv_right_tot = 0

			# where calculating velocity
			lata_rad = math.radians(lata[jobs]); lona_rad = math.radians(lona[jobs]);
			latb_rad = math.radians(latb[jobs]); lonb_rad = math.radians(lonb[jobs]);
			lato_rad = math.radians(lato[jobs]); lono_rad = math.radians(lono[jobs])
			azimuth = alpha[jobs]; 
			...
			for iset in range(n_segs):  # segments driving the velocity/pressure field
				...
				# where driving the velocity
				lonaa = lona[iset];  lataa = lata[iset]
				lonbb = lonb[iset];  latbb = latb[iset]
				gm  = gam[iset]; alp = alpha[iset] 	

				if iwall[iset] == 1 and iset >= num_segs: # wall component

					# right side of boundary
					side = 1;	obs_azim = azimuth + (np.pi/2.)
					lonobsa, latobsa = project_to_point(lona_rad, lata_rad, obs_azim, len_ratio_lit)			
					lonobsb, latobsb = project_to_point(lonb_rad, latb_rad, obs_azim, len_ratio_lit)			
					deriv_right = calcvel_wall(math.degrees(lonobsa),math.degrees(latobsa),math.degrees(lonobsb),math.degrees(latobsb),azimuth,gamma,\
						lonaa,lataa,lonbb,latbb,gm,alp,iset,thresh_dist_wall,elit,ebig,side,taper_length,rad_km)
					...
					lonobsa, latobsa = project_to_point(lona_rad, lata_rad, obs_azim, len_ratio_lit)			
					lonobsb, latobsb = project_to_point(lonb_rad, latb_rad, obs_azim, len_ratio_lit)			
					deriv_left = calcvel_wall(math.degrees(lonobsa),math.degrees(latobsa),math.degrees(lonobsb),math.degrees(latobsb),azimuth,gamma,\
						lonaa,lataa,lonbb,latbb,gm,alp,iset,thresh_dist_wall,elit,ebig,side,taper_length,rad_km)
					...
					obs_azim = azimuth + (np.pi/2.)
					lonobs , latobs = project_to_point(lono_rad, lato_rad, obs_azim, len_ratio_big)			
					fpress1 = findpressure_edge(math.degrees(lonobs),math.degrees(latobs),lonaa,lataa,lonbb,latbb,gm,rad_km)
					# right side (closer to boundary)
					lonobs , latobs = project_to_point(lono_rad, lato_rad, obs_azim, len_ratio_lit)			
					fpress = findpressure_edge(math.degrees(lonobs),math.degrees(latobs),lonaa,lataa,lonbb,latbb,gm,rad_km)
					deriv_right = (fpress1 - fpress)/(ebig - elit) 	# right direction positive

					# left side (closer to boundary)
					obs_azim = azimuth - (np.pi/2.)
					lonobs , latobs = project_to_point(lono_rad, lato_rad, obs_azim, len_ratio_lit)			
					fpress3 = findpressure_edge(math.degrees(lonobs),math.degrees(latobs),lonaa,lataa,lonbb,latbb,gm,rad_km)
					# left side (further from boundary)
					lonobs , latobs = project_to_point(lono_rad, lato_rad, obs_azim, len_ratio_big)			
					fpress = findpressure_edge(math.degrees(lonobs),math.degrees(latobs),lonaa,lataa,lonbb,latbb,gm,rad_km)
					deriv_left = (fpress3 - fpress)/(ebig-elit) 	# right direction positive


				deriv_right_tot = deriv_right_tot + (deriv_right * pcoeff[iset])	# Pa/m
				deriv_left_tot  = deriv_left_tot  + (deriv_left * pcoeff[iset])	# Pa/m
			...
			# idr: dP coefficient for right side of boundary
			if ndomain[idr[jobs]-1] == 100 or ndomain[idr[jobs]-1] == 500:	# fixed plate, free base
				deriv_right_tot_converted = (deriv_right_tot/3.) * co1 * second_coeff_freebase;
			elif ndomain[idr[jobs]-1] == 400:  					# fixed plate, free slab stail
				deriv_right_tot_converted = (deriv_right_tot/3.) * co2 * second_coeff_freebase2;
			elif ndomain[idr[jobs]-1] == 200:					# fixed plate, fixed slab tail
				deriv_right_tot_converted = (deriv_right_tot/12.) * co2 * second_coeff_fixedbase2;
			elif ndomain[idr[jobs]-1] == 300:					# fixed plate, fixed base
				deriv_right_tot_converted = (deriv_right_tot/12.) * co1 * second_coeff_fixedbase;

			# idl: dP coefficient for left side of boundary
			if ndomain[idl[jobs]-1] == 100 or ndomain[idl[jobs]-1] == 500:	# fixed plate, free base
				deriv_left_tot_converted = (deriv_left_tot/3.) * co1 * second_coeff_freebase;
			elif ndomain[idl[jobs]-1] == 400:  					# fixed plate, free slab stail
				deriv_left_tot_converted = (deriv_left_tot/3.) * co2 * second_coeff_freebase2;	
			elif ndomain[idl[jobs]-1] == 200:					# fixed plate, fixed slab tail
				deriv_left_tot_converted = (deriv_left_tot/12.) * co2 * second_coeff_fixedbase2;
			elif ndomain[idl[jobs]-1] == 300:					# fixed plate, fixed base
				deriv_left_tot_converted = (deriv_left_tot/12.) * co1 * second_coeff_fixedbase;

			# idr: vp coefficients for right side of boundary
			if ndomain[idr[jobs]-1] == 100 or ndomain[idr[jobs]-1] == 400 or ndomain[idr[jobs]-1] == 500: 	# fixed plate, free base
				right_vtop_coeff  = 1.0
				right_vbott_coeff = 0
			else:															# fixed plate, fixed base
				right_vtop_coeff  = (1./6.)*((6.*rad_km + asth_thick)/(2.*rad_km - asth_thick))
				right_vbott_coeff = (1./6.)*((6.*rad_km - asth_thick)/(2.*rad_km - asth_thick))
			# idl: vp coefficients for right side of boundary
			if ndomain[idl[jobs]-1] == 100 or ndomain[idl[jobs]-1] == 400 or ndomain[idl[jobs]-1] == 500: 	# fixed plate, free base
				left_vtop_coeff  = 1.0
				left_vbott_coeff = 0
			else:															# fixed plate, fixed base
				left_vtop_coeff  = (1./6.)*((6.*rad_km + asth_thick)/(2.*rad_km - asth_thick))
				left_vbott_coeff = (1./6.)*((6.*rad_km - asth_thick)/(2.*rad_km - asth_thick))		

			vp_component_right = vtopr[jobs]*right_vtop_coeff + vbotr[jobs]*right_vbott_coeff
			vp_component_left  = vtopl[jobs]*left_vtop_coeff  + vbotl[jobs]*left_vbott_coeff
			vt_right = (-1.0 * deriv_right_tot_converted + vp_component_right) * vel_convert   # cm/yr
			vt_left  = (-1.0 * deriv_left_tot_converted  + vp_component_left) * vel_convert	  # cm/yr
			vt_avg = 0.5*(vt_left+vt_right)

			if polarity[jobs] == 1: # dips to left
				predicted_vts[jobs,:] = large_wall_inds[jobs], lono[jobs], lato[jobs], -1.0*vt_right,-1.0*vt_left, vt[jobs]*vel_convert
				print "wall ind %.0f: va_sp    = %.2f, va_op    = %.2f cm/yr, vact = %.2f cm/yr" % (large_wall_inds[jobs],-1.0*vt_right,-1.0*vt_left,vt[jobs]*vel_convert)
			elif polarity[jobs] == 2: # dips to right
				predicted_vts[jobs,:] = large_wall_inds[jobs], lono[jobs], lato[jobs], vt_left, vt_right, vt[jobs]*vel_convert
				print "wall ind %.0f: va_sp    = %.2f, va_op    = %.2f cm/yr, vact = %.2f cm/yr" % (large_wall_inds[jobs],vt_left,vt_right,vt[jobs]*vel_convert)

	return predicted_vts
