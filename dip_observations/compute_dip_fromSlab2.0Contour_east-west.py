...
lons=np.arange(lon_min,lon_max,(lon_max-lon_min)/300.);
dips_tmp=np.zeros([len(lons),5])

# normalize box width so it's an ~equivalent degree distance
box_width=int(round(box_width*(10./(lon_max-lon_min))))

net_distance=0; num_bad = 0
for n in range(len(lons)):

		dlon = 90.0; ind_trench = 100;
		for i in range(len(trench_loc)):
			dlon_tmp = np.abs(lons[n] - trench_loc[i,0]);
			if dlon_tmp < dlon:
				dlon = dlon_tmp;
				ind_trench = i;
		...
		dips_tmp[n,:] = (trench_loc[ind_trench,0],trench_loc[ind_trench,1],slab_dip,dang,dlon);

		if dang >= 5 or dlon >= 0.1:
			num_bad = num_bad + 1

# get rid of bad points
dips=np.zeros([len(lons)-num_bad,3]); ind=0
for n in range(len(lons)):
...
if smooth==1:
	...
	dips[2:(np.size(dips,0)-2),2]=smoothed_dips[2:(np.size(dips,0)-2)]
